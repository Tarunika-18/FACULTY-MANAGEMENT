...
def get_current_user():
    ...
    if 'user' in session:
        user = session['user']
        db = get_database()
        user_cur = db.execute('select * from users where email = ?', [user])
...
def dashboard():
    ...
    db = get_database()
    if 'user' in session:
        emp_cur = db.execute('select * from emp where email = ?', [user])
        myself = emp_cur.fetchone()
    return render_template('dashboard.html', user = user, myself=myself)
